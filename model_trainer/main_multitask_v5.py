...
class TaskSpec:
    ...
    - For this script's current datasets:
      - ELM classification labels come as a dict keyed by quantile (e.g. 0.5).
            - ELM regression labels can be sourced from the same dict (e.g., time_to_elm).
      - Confinement labels are integer class indices.
    """

    name: str = None
    task_type: Literal["binary", "multiclass", "regression"] = 'binary'
    head_layers: Sequence[int] = (32,)
    label_format: Literal["quantile_to_logit_map", "multiclass_index", "scalar"] = "quantile_to_logit_map"
    ...
    track_elmwise_f1: Optional[bool] = False
...
def _validate_task_specs(task_specs: Sequence[TaskSpec]) -> tuple[TaskSpec, ...]:
    ...
    if not all([getattr(spec, 'name', None) for spec in task_specs]):
...
class Model(_Base_Class, LightningModule):
    ...
    def __post_init__(self):
        ...
        if self.task_specs is None or len(self.task_specs)==0:
            raise ValueError("task_specs must be provided with at least one task")
                ...
                for metric_name in score_metric_names:
                    ...
                    if spec.task_type == 'binary':
                        ...
                        if metric_name == 'r2_score':
                            ...
                            raise ValueError(f"Unknown regression score metric: {metric_name}")
                    else:
                        raise ValueError(f"Unsupported task_type: {spec.task_type}")
    ...
    def update_step(
        ...
        for task in model_outputs:
            ...
            if spec.label_format == 'quantile_to_logit_map':
            ...
            elif spec.label_format == 'scalar':
                raw = task_batch[1]
                if isinstance(raw, dict):
                    if 'time_to_elm' in raw:
                        labels = raw['time_to_elm']
                    elif 'scalar' in raw:
                        labels = raw['scalar']
                    else:
                        raise ValueError(
                            f"Task {task!r} requested label_format='scalar' but batch labels dict had keys {list(raw.keys())}"
                        )
                else:
                    labels = raw
                # Regression outputs should be shaped like labels (B,) or (B,1)
                task_outputs = task_outputs.reshape_as(labels)
            else:
                ...
                if 'loss' in metric_name:
                    if metric_name == 'bce_loss':
                        ...
                        metric_value = metric_function(
                            input=task_outputs,
                            target=(labels.type_as(task_outputs) if spec.task_type in ('binary', 'regression') else labels),
                        )
            ...
            if stage in STAGE_KEYS_TRAIN_VAL_TEST:
                if spec.task_type == 'binary':
                    ...
                    preds_for_metrics = task_outputs.to(dtype=torch.float32).reshape(-1)
                    target_for_metrics = labels.to(dtype=torch.float32).reshape(-1)
                else:
                    raise ValueError(f"Unsupported task_type: {spec.task_type}")
    ...
    def on_predict_end(self) -> None:
        ...
        for i_dl, batch_list in self.predict_outputs.items():
            ...
            if task == 'elm_class':
                ...
                predictions = self._expit_np(outputs)
                ...
                for i in range(num_classes):
                    ...
                    probs = self._softmax_np(outputs, axis=1)[:, i]
                    sm_probs = self._softmax_np(smoothed_outputs, axis=1)[:, i]
                ...
                # Plotting is currently only implemented for the built-in classification tasks.
                self.rprint(f"  Skipping prediction plot for task {task!r}")
                continue
            plt.tight_layout()
...
class Data(_Base_Class, LightningDataModule):
    ...
    def __post_init__(self):
        ...
        self.has_elm_regression = any(spec.name == 'elm_regression' for spec in self.task_specs)
        self.has_conf_onehot = any(spec.name == 'conf_onehot' for spec in self.task_specs)
        self.has_elm_data = self.has_elm_class or self.has_elm_regression
        ...
        supported = {'elm_class', 'elm_regression', 'conf_onehot'}
        for task_name in (spec.name for spec in self.task_specs):
            assert task_name in supported, f"Unknown task {task_name}. Supported: {sorted(supported)}"
    ...
    def _validate_sub_stage_key(stage: str) -> str:
        STAGE_KEYS = ('train', 'val', 'test', 'predict')
    ...
    def setup(self, stage: str):
        ...
        sub_stages = tuple(self._validate_sub_stage_key(s) for s in sub_stages)
        if self.has_elm_data:
            t_tmp = time.time()
        ...
        for sub_stage in sub_stages:
            ...
            for spec in self.task_specs:
                ...
                datasets = (
                    ...
                    if task in ('elm_class', 'elm_regression')
    ...
    def setup_elm_data_for_rank(self, sub_stage: str):
        sub_stage = self._validate_sub_stage_key(sub_stage)
    ...
    def get_dataloader_from_dataset(
        ...
        stage = self._validate_sub_stage_key(stage)
        batch_size: int = None
    ...
    def set_fir_filter(self):
        ...
        if self.fir_bp[0] or self.fir_bp[1]:
            ...
            if f_low and f_high:
                ...
                pass_zero = 'lowpass'
                cutoff = f_high
            self.b_coeffs = self._firwin_windowed_sinc(
    ...
    def apply_fir_filter(self, signals: np.ndarray) -> np.ndarray:
        ...
        return self._lfilter_fir_causal(x_np, b_np)
    ...
    def train_dataloader(self) -> CombinedLoader:
        ...
        for spec in self.task_specs:
            ...
            elif spec.name == 'elm_regression':
                loaders['elm_regression'] = self.get_dataloader_from_dataset('train', self.elm_datasets['train'])
    ...
    def val_dataloader(self) -> CombinedLoader:
        ...
        for spec in self.task_specs:
            if spec.name == 'elm_class':
                ...
                loaders['elm_regression'] = self.get_dataloader_from_dataset('val', self.elm_datasets['val'])
            elif spec.name == 'conf_onehot':
    ...
    def test_dataloader(self) -> CombinedLoader:
        ...
        for spec in self.task_specs:
            if spec.name == 'elm_class':
                ...
                loaders['elm_regression'] = self.get_dataloader_from_dataset('test', self.elm_datasets['test'])
            elif spec.name == 'conf_onehot':
...
class ELM_TrainValTest_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i: int) -> tuple:
        ...
        labels = {q: int(sw_metadata['time_to_elm'] <= qval) for q, qval in self.time_to_elm_quantiles.items()}
        # Also expose a scalar regression target (time to ELM in ms) via the same labels dict.
        labels['time_to_elm'] = float(sw_metadata['time_to_elm'])
        return signal_window, labels, sw_metadata['elm_index']
...
def main(
    ...
    if task_specs is None or len(task_specs)==0:
        task_specs = (
            TaskSpec(
                name='elm_class',
                task_type='binary',
                label_format='quantile_to_logit_map',
                head_layers=(16,),
                label_quantile=0.5,
                dataloader_idx=0,
    ...
    task_specs = _validate_task_specs(task_specs)
...
if __name__=='__main__':
    ...
    task_specs = (
        ...
        TaskSpec(
            ...
            head_layers=(16,),
            # Uses the scalar time-to-ELM label (ms) exposed by ELM_TrainValTest_Dataset.
            label_format='scalar',
            monitor_metric='r2_score/val',
            dataloader_idx=2,
        ),
    )
    main(
