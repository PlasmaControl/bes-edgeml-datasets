...
Example
  python -m model_trainer.main_multitask_v6 --n_time 256 --kernel_t 8 --max_epochs 5
    ...
    def __init__(
        ...
        *,
        ...
        for c_out in self.channels:
            ...
            blocks.append(nn.LeakyReLU(negative_slope=leaky_relu_slope, inplace=True))
            c_in = c_out
        ...
        self.to_latent = nn.Sequential(
            nn.Linear(self.channels[-1], int(latent_dim)),
    ...
    *,
    ...
    for h in hidden_dims:
        ...
        if dropout and dropout > 0:
            layers.append(Dropout1dFeatures(p=float(dropout)))
        prev = h
...
class MultiTask3DDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        # x: [N, 1, T, 8, 8]
        self.x = torch.randn(self.num_samples, 1, self.n_time, 8, 8)
        ...
        for spec in self.task_specs:
            if spec.task_type == "regression":
                ...
                logits = shared.unsqueeze(-1).repeat(1, spec.output_dim)
                ...
                class_logits = class_logits + 0.5 * torch.randn_like(class_logits)
...
class Model(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        self.task_names = [t.name for t in self.task_specs]
        ...
        self.backbone = Conv3DBackbone(
            in_channels=1,
            kernel_t=int(kernel_t),
            channels=tuple(int(c) for c in backbone_channels),
        ...
        # Note: We still compute differentiable losses via torch.nn.functional for backprop.
        # These metrics are for logging/aggregation.
        # TorchMetrics will synchronize state across processes when compute() is called
        # (Lightning triggers this for on_epoch logging). We also pass sync_dist=True
        # when logging to ensure correct DDP reduction behavior.
        metric_kwargs = {
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    p.add_argument("--data_path", type=str, default=None, help="Optional dataset path (.pt).")

    p.add_argument("--n_time", type=int, default=256, help="Time length (>=128) for inputs [8,8,n_time].")
    p.add_argument("--kernel_t", type=int, choices=(4, 8), default=8, help="Temporal kernel size (4 or 8).")
    p.add_argument(
        "--backbone_channels",
        type=str,
        default="16,32,64",
        help="Comma-separated conv channels, e.g. '16,32,64'.",
    )
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Task specs as JSON string or path to .json file. "
            "Each entry: {name, task_type in [binary,multiclass,regression], output_dim, head_hidden_dims, head_dropout}."
        ),
    )
    ...
    if int(args.n_time) < 128:
        raise ValueError("--n_time must be >= 128")

    task_specs = _parse_task_specs(args.tasks_json)
    ...
    logger = TensorBoardLogger(save_dir=str(args.log_dir), name="multitask_v6")
